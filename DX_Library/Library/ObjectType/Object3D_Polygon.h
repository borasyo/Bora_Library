...
public:
	...
	void Draw( GameObject* pObj, const D3DXVECTOR3 Pos, const D3DXVECTOR3 Size, const D3DXVECTOR3 Rot );
	const LPCWSTR GetFileName();
	void SetFileName( const LPCWSTR& filename );
	void SetBillFlg( const bool bFlg = false );
	void SetDivide( const D3DXVECTOR3 Divide );

	// 描画方法セット
	void SetSynthetic(ESynthetic Synthetic);
	...
	void SetVertexTexture( const D3DXVECTOR3 Pattern ); // 分割数、何パターン目か
};
