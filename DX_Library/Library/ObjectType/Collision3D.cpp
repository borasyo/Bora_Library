...
bool Collision3D::CollisionPointandCircle( GameObject* pPoint, GameObject* pCircle ){
	...
	if( sqrt( fDistanceX*fDistanceX + fDistanceY*fDistanceY + fDistanceZ*fDistanceZ ) 
		<= pCircle->GetCollisionCircleSize() / 2.0f ) {
...
bool Collision3D::CollisionCubeandPoint( GameObject* pCube, GameObject* pPoint ) {
	...
	if( ppos.x < cpos.x + ccolplus.x  &&
		...
		ppos.z < cpos.z + ccolplus.z  &&
		ppos.z > cpos.z + ccolminus.z ) 
	{
...
bool Collision3D::CollisionCubeandCube( GameObject* pCube, GameObject* pCube2 ) {
	...
	// あまりに遠いオブジェクトは判定外に
	//float fDistance;
	D3DXVECTOR3 pos[2] = {*(pCube->GetPosition()), *(pCube2->GetPosition())};
	float fDistanceX = abs(pos[0].x - pos[1].x);
	float fDistanceY = abs(pos[0].y - pos[1].y);
	float fDistanceZ = abs(pos[0].z - pos[1].z);
	...
	//sD3DXVECTOR3 ccolminus[2] = {pCube->GetCollisionCubeMinusSize()/2.0f, pCube2->GetCollisionCubePlusSize()/2.0f};
	if( ccolplus[0].x + ccolplus[1].x >= fDistanceX && 
		ccolplus[0].y + ccolplus[1].y >= fDistanceY && 
		ccolplus[0].z + ccolplus[1].z >= fDistanceZ ) {
...
bool Collision3D::CollisionCubeandCircle( GameObject* pCube, GameObject* pCircle ) {
	...
	D3DXVECTOR3 pos[2] = {*(pCube->GetPosition()), *(pCircle->GetPosition())};
	float fDistanceX = pos[0].x - pos[1].x;
	float fDistanceY = pos[0].y - pos[1].y;
	float fDistanceZ = pos[0].z - pos[1].z;
	
	// 2点間の距離が２つのオブジェクトの半径の合計より近いか
	//fDistance = sqrt( fDistanceX*fDistanceX + fDistanceY*fDistanceY + fDistanceZ*fDistanceZ );
	
	float ccolcircle = pCircle->GetCollisionCircleSize() * fCircleSize / 2.0f;
	D3DXVECTOR3 ccolplus = pCube->GetCollisionCubePlusSize() / 2.0f; 
	//D3DXVECTOR3 ccolminus = pCube->GetCollisionCubeMinusSize() / 2.0f;
	if( ccolplus.x + ccolcircle >= fDistanceX && 
		ccolplus.y + ccolcircle >= fDistanceY && 
		ccolplus.z + ccolcircle >= fDistanceZ ) {
