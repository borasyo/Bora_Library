...
HRESULT CDirect3D::InitDirect3D(){
	...
	if( m_bWindowMode ){
		...
		d3dpp.BackBufferHeight = (UINT)m_pWindow->GetClientHeight();
		d3dpp.BackBufferWidth  = (UINT)m_pWindow->GetClientWidth();
...
void CDirect3D::InitRender( void ){
	...
	m_pD3DDevice->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE);	// ２番目のアルファ引数
	...
	D3DXMatrixPerspectiveFovLH( &matProj, D3DX_PI/4, (float)window->GetWindowWidth()/(float)window->GetWindowHeight(), 1.f, 1000.f );
